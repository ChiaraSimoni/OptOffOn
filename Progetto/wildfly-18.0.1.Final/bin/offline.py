...
def main(argv):
    ...
    if uchp_index_list:
        ...
        model.Tildeuchp = Var(fixed_time_list, fixed_scenario_list, domain = NonNegativeReals) 
    ...
    if gas_chp_index_list:
        ...
        model.Tildegas_chp = Var(fixed_time_list, fixed_scenario_list, domain = NonNegativeReals) 
    ...
    for i in uchp_index_list:
        for s in fixed_scenario_list:
            ...
            for t in range(8*multipl_constant, 12*multipl_constant):
                model.bound_Suchp.add(inequality(-0.5, model.Suchp[i,t],0.5))
    ...
    for i in gas_chp_index_list:
        for s in fixed_scenario_list:
            for t in range(0*multipl_constant, 4*multipl_constant):
                model.bound_Sgas_chp.add(inequality(-2, model.Sgas_chp[i,t], -2))
                if t < 4*multipl_constant - 1:
            ...
            for t in range(12*multipl_constant, 16*multipl_constant):
                model.bound_Sgas_chp.add(inequality(0, model.Sgas_chp[i,t], 0))
                if t < 16*multipl_constant - 1:    
                    #model.bound_Sgas_chp.add( model.Tildegas_chp[t,s] == model.Tildegas_chp[t+1,s])
                    model.bound_Sgas_chp.add( model.Sgas_chp[i,t] == model.Sgas_chp[i,t+1])
                    model.bound_Sgas_chp.add( model.Pgas_chp[i,t,s] == model.Pgas_chp[i,t+1,s])
            ...
            for t in range(16*multipl_constant, 20*multipl_constant):
                model.bound_Sgas_chp.add(inequality(-0.5, model.Sgas_chp[i,t], 0.5))
                if t < 20*multipl_constant - 1:
                   #model.bound_Sgas_chp.add( model.Tildegas_chp[t,s] == model.Tildegas_chp[t+1,s])
                   model.bound_Sgas_chp.add( model.Sgas_chp[i,t] == model.Sgas_chp[i,t+1])
                   model.bound_Sgas_chp.add( model.Pgas_chp[i,t,s] == model.Pgas_chp[i,t+1,s])
    ...
    for t in fixed_time_list:
        for s in fixed_scenario_list:
            ...
            model.bound_tildeload.add(left_side - right_side == 0) 
    ...
    for t in fixed_time_list:
        for s in fixed_scenario_list:
            ...
            model.bound_tildeload2.add(left_side - right_side == 0)
    
    '''model.bound_tildeload3 = ConstraintList()
    for t in fixed_time_list:
        for s in fixed_scenario_list:
            right_side = 0
            left_side = 0
            for i, pod in enumerate(pod_list_conf):
                ...
                if 'load' in pod[2]:
                    if S == 1:
                        left_side += model.Sload[i,t] + pod[3]['load'].values.tolist()[t]
                    elif S == 2:
                        if s == 0:
                            left_side += model.Sload[i,t] + pod[3]['load+'].values.tolist()[t]
                        else:
        ...
        model.bound_tildeload3.add(left_side - right_side == 0)'''
    #SLOAD:
    if load_index_list:    
    ...
    for t in fixed_time_list:
        acc = 0
        for i in load_index_list:
            acc += model.Sload[i,t].value
        plot = float(acc/(S))
        sloadlist.append(plot)
    ...
    images, = result.plot(tildelist, linestyle='-', color='brown', label='Load')
    ...
    plt.grid(True)
    resultimg = plt.savefig('results1_offline_{}.png'.format(argv[3]), dpi=200) 
    ...
    plt.locator_params(axis='x', nbins=24)
    plt.grid(True)
    result2img = plt.savefig('results2_offline_{}.png'.format(argv[3]), dpi=200) 
    ...
    ax = plt.savefig('pie_offline_{}.png'.format(argv[3]), dpi=200) 
    ...
    profileimg, profileplt = plt.subplots(figsize=(15, 10))
    images, = profileplt.plot(loadlist, linestyle='-', color='blue', linewidth=2)
    images, = profileplt.plot(tildelist, linestyle='-', color='green', linewidth=2)
    images, = profileplt.plot(flattened_profile, linestyle='-', color='red', linewidth=2)
    ...
    plt.locator_params(axis='x', nbins=24)
    plt.grid(True)
    uchpimg = plt.savefig('results_offline_desired_profile.png', dpi=200) 
    
    plt.close(profileimg)
